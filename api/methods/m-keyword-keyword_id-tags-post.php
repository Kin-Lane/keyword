<?php
$route = '/keyword/:keyword_id/tags/';
$app->post($route, function ($Keyword_ID)  use ($app){
	...
	if(isset($param['tag']))
		...
		$CheckTagPivotQuery = "SELECT * FROM keyword_tag_pivot where Tag_ID = " . trim($Tag_ID) . " AND Keyword_ID = " . trim($Keyword_ID);
		...
		if($CheckTagPivotResult && mysql_num_rows($CheckTagPivotResult))
			...
			{
			$query = "INSERT INTO keyword_tag_pivot(Tag_ID,Keyword_ID) VALUES(" . $Tag_ID . "," . $Keyword_ID . "); ";
			mysql_query($query) or die('Query failed: ' . mysql_error());
			}
